...
class OwlViTModel(BaseModel):
    ...
    def forward(self, image: torch.Tensor, text: List[str], return_labels: bool = False):
        ...
        if isinstance(text, str):
            text = [text]
        text_original = text
        texts = ['a photo of a ' + t for t in text]

        inputs = self.processor(text=[texts], images=image, return_tensors="pt")
        inputs = {k: v.to(self.dev) for k, v in inputs.items()}
        outputs = self.model(**inputs)

        if isinstance(image, Image.Image):  # PIL Image check
            target_sizes = torch.Tensor([image.size[::-1]]).to(self.dev)
        elif isinstance(image, torch.Tensor):  # PyTorch Tensor check
            target_sizes = torch.Tensor([image.size()[1:3]]).to(self.dev)  # Extract (height, width)
...
class TCLModel(BaseModel):
    ...
    def __init__(self, gpu_number=0):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(text_encoder, device_map="auto")
...
class GPT3Model(BaseModel):
    ...
    def query_gpt3(self, prompt, model="gemini", max_tokens=16, logprobs=None, stream=False,
                   stop=None, top_p=1, frequency_penalty=0, presence_penalty=0):
        
        aiplatform.init(project="glp2024-450517", location="us-central1")

        model = GenerativeModel("gemini-1.5-flash-002")
        
        system_prompt = "Please answer the following question without any visual context:"
...
def codex_helper(extended_prompt):
    print(f"Chosen code generation model is: {config.codex.model}")
    ...
    #         if len(resp) == 1:
    #             resp = resp[0]
    if config.codex.model in ("hfchat"):
        if not isinstance(extended_prompt, list):
            extended_prompt = [extended_prompt]
        sign = Login(config.codex.hf_chat_login, config.codex.hf_chat_pw)
        cookies = sign.login(cookie_dir_path=cookie_path_dir, save_cookies=True)
        system_prompt = "You are a helpful assistant. Only answer with a function starting def execute_command, which can take only arguments: image, my_fig, time_wait_between_lines, syntax"
        chatbot = hugchat.ChatBot(cookies=cookies.get_dict(), system_prompt=system_prompt)
        chatbot.switch_llm(2)
        info = chatbot.get_conversation_info()
        print(f'Chosen HuggingChat model is: {info.model}')

        responses = [chatbot.chat(prompt).wait_until_done()
                     for prompt in extended_prompt]
        resp = [r.replace("```python\n", "").replace("\n```", "") for r in responses]

        chatbot.delete_all_conversations()
        chatbot.new_conversation(switch_to=True,system_prompt=system_prompt)
    
    if config.codex.model in ("yi-coder"):
        ...
        messages = [
            {"role": "system", "content": "You are a helpful assistant. \
            Respond with only a single Python function 'def execute_command'. \
            The function must take only the following arguments: \
            image, my_fig, time_wait_between_lines, syntax. The response must contain only \
            the syntactically correct python function body without any comments, explanations, \
            or nested functions."},
            {"role": "user", "content": extended_prompt[0]}
            ]
    ...
    if config.codex.model in ("gemini"):

        aiplatform.init(project="glp2024-450517", location="us-central1")

        model = GenerativeModel("gemini-1.5-flash-002")

        system_prompt = "Respond with only a single Python function 'def execute_command'. The function must take only the following arguments: image, my_fig, time_wait_between_lines, syntax."
        full_prompt = system_prompt + "\n" + extended_prompt[0]
    ...
    def forward(self, prompt, input_type='image', prompt_file=None, base_prompt=None, extra_context=None):
        ...
        if isinstance(prompt, list):
            ...
            extended_prompt = [base_prompt.replace("INSERT_QUERY_HERE", p).
                               ...
                               for p, ec in zip(prompt, extra_context or [None] * len(prompt))]
            ...
            if extra_context is not None:
                extended_prompt = [extended_prompt.replace("EXTRA_CONTEXT_HERE", extra_context)]
...
class BLIPModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, half_precision=config.blip_half_precision,
        ...
        with warnings.catch_warnings(), HiddenPrints("BLIP"), torch.cuda.device(self.dev):
            ...
            self.processor = Blip2Processor.from_pretrained(f"Salesforce/{blip_v2_model_type}", device_map="auto")
    ...
    def qa(self, image, question):
        ...
        generated_ids = self.model.generate(**inputs, length_penalty=-1, num_beams=5, max_length=64, min_length=1,
                                            do_sample=False, top_p=0.9, repetition_penalty=1.0,
                                            num_return_sequences=1, temperature=1)
...
class XVLMModel(BaseModel):
    ...
    def __init__(self, gpu_number=0,
        ...
        self.model = model
        self.tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', device_map="auto")
