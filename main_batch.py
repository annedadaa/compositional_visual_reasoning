...
def run_program(parameters, queues_in_, input_type_, retrying=False):
    ...
    updated_code = re.sub(r'def execute_command\((.*?)\):', code_header, code, count=1)
    ...
    if config.save:
        ...
        if not config.save_new_results:
            ...
            if len(existing_files) == 0:
                filename = 'results_0_' + os.path.basename(config.dataset.data_path) + '.csv'
            else:
                filename = 'results_' + os.path.basename(config.dataset.data_path) + '.csv'
