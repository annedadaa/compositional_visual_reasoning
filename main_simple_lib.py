...
def get_code(query):
    ...
    # print('code:', code)
    # if config.codex.model not in ('gpt-3.5-turbo', 'gpt-4', 'hfchat', 'yi-chat'):
    ...
    if config.codex.model in ('hfchat', 'yi-coder', 'gemini'):
        code_for_syntax = code.replace("(image, my_fig, time_wait_between_lines, syntax)", "(image)")
        pattern = r'def execute_command\(.*?\)'
        replacement = "def execute_command(image, my_fig, time_wait_between_lines, syntax)"
